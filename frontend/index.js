...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon-api-worker.tolujohn.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
            ...
            if (status === 200) {
                ...
                throw new Error("Worker Err" + data)
            }
        }))
...
async function fetchReport(data) {
    ...
    try {
        const url = "https://openai-api-worker.tolujohn.workers.dev"
